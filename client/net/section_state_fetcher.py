...
class SectionStateFetcher:
    """
    Responsible for fetching the state of a section.
    """
    ...
    def get_main_section_state(self) -> WorldState:
        ...
        if not self.net_client.reader.data_available():
            raise Exception("No response from the server about the main section state.")

        datagram = NetDatagram()

        if not self.net_client.reader.get_data(datagram):
            raise Exception("Main section state response unavailable.")
